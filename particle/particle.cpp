...
bool loadtexture()                                    // Load Bitmaps And Convert To Textures
{
		...
		TextureImage[0]->data = new unsigned char[TextureImage[0]->sizeX*TextureImage[0]->sizeY*3];
...
void render(void *esContext)
{
	...
	for (loop=0; loop<num; loop++)						// Loop Through All The Stars
	{
		...
		GLfloat color[] = {star[loop].r/255.0f,star[loop].g/255.0f,star[loop].b/255.0f,255/255.0f};
		...
		if(0)
		{//dump framebuffer through glReadPixels
			unsigned char* read_buffer = new unsigned char[DISPLAY_WIDTH*DISPLAY_HEIGHT*4];
			glReadPixels(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE,read_buffer);
			FILE* file = fopen("swapbuffer.bin", "ab");
			if (file)
			{
				fwrite(read_buffer,1, DISPLAY_WIDTH*DISPLAY_HEIGHT*4, file);
				fclose(file);
			}
			delete read_buffer;
		}
...
int main(int argc, char **argv)
{
	int q;
	int start, end;
	printf("Initializing GLES 1.1 & EGL pure software stack for test sample!\n\n");
	printf("The GLES 1.1 & EGL pure software stack is encapsulated in swrender.dll. \n\n");
