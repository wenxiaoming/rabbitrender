...
static size_t dataSizePalette4(int numLevels, int width, int height, int format)
{
    ...
    for (int i=0 ; i< numLevels ; i++) {
		int w = (width  >> i) ? (width  >> i): 1;
		int h = (height >> i) ? (height >> i): 1;

        int levelSize = h * ((w * indexBits) / 8) ? h * ((w * indexBits) / 8) : 1;
        size += levelSize;
    }
...

{
	...
	width  = (width  >> level) ? (width  >> level): 1;
	height = (height >> level) ? (height >> level): 1;
        ...
        GLsizei imageSize, const GLvoid *data)
{
    ...
    for (int i=0 ; i<numLevels ; i++) {
		int lod_w = (width  >> i) ? (width  >> i): 1;
		int lod_h = (height >> i) ? (height >> i): 1;
        int error = createTextureSurface(c, &surface, &size,
