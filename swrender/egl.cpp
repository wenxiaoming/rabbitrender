...
static pthread_key_t gEGLErrorKey = (pthread_key_t)-1;
#ifndef HAVE_ANDROID_OS
namespace gl {
pthread_key_t gGLKey = (pthread_key_t)-1;
}; // namespace gl
#endif
...
static T setError(GLint error, T returnValue) {
    if (ggl_unlikely(gEGLErrorKey == (pthread_key_t)-1)) {
        ...
        if (gEGLErrorKey == (pthread_key_t)-1)
...
private:
    ...
    struct Rect {
        ...
        void dump(char const* what) {
            printf("%s { %5d, %5d, w=%5d, h=%5d }",what, left, top, right-left, bottom-top);
        }
        ...
        NativePixmapType pixmap, const EGLint *attrib_list)
{
    ...
    if (((egl_native_pixmap_t*)(pixmap))->version != 
    ...
    if (((egl_native_pixmap_t*)pixmap)->format != pixelFormat)
        return setError(EGL_BAD_MATCH, EGL_NO_SURFACE);
...
void setCPUAffinity()
{
	...
	{
		SetProcessAffinityMask(currentProcess, (ProcessAffinityMask&0x01));
	}
