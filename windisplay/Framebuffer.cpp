...
Framebuffer::Framebuffer(int32_t width, int32_t height, int32_t format)
    : mWindowWidth(width), mWindowHeight(height), mFormat(format) {
    ...
    for (i = 0; i < mNumBuffers; i++) {
        buffers[i] = new NativeBuffer(mWindowWidth, mWindowHeight, mFormat,
                                      GRALLOC_USAGE_HW_FB);
    }
    ...
    for (i = 0; i < mNumBuffers; i++) {
        ...
        buffers[i]->handle->data[0] =
            (uintptr_t)((uint8_t *)malloc(mWindowWidth * mWindowHeight * 4));
...
int Framebuffer::setSwapInterval(ANativeWindow *window, int interval) {
    return 0;
}
...
int Framebuffer::queueBuffer(ANativeWindow *window,
    ...
    Framebuffer *self = (Framebuffer *)window;
    const int index = self->mCurrentBufferIndex;
    if (0) { // dump output framebuffer to file
        FILE *file = fopen("datadump.bin", "ab");
        if (file) {
            fwrite((uint8_t *)(buffer->handle->data[0]), 1,
                   buffer->height * buffer->width * 4, file);
            fclose(file);
        }
    }
...
int Framebuffer::query(const ANativeWindow *window, int what, int *value) {
    Framebuffer *self = (Framebuffer *)window;
    switch (what) {
...
int Framebuffer::perform(ANativeWindow *window, int operation, ...) {
    return 0;
}
